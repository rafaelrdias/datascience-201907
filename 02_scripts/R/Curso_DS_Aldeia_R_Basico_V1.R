...
vendas <- data.frame(vendedor=c(13,26,39,39,07,14,13,07,21),
                     vendas=c(300,500,110,150,280,580,90,20,800))

vendas

metas <- data.frame(vendedor=c(07,13,14,21,26,39),
                    meta=c(2000,800,1250,1050,1400,1550))
...
result_mes$ating_meta <- ifelse(result_mes$vendas>=result_mes$meta,
                                "SIM",
                                "NÃO")
...
# Ajustando um modelo de regressão linear >> "y = f(x)" or "y = B0 + (B1 * x)"
